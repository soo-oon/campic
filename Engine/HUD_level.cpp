...
void HUD_level::Update(float /*dt*/)
{
	...
	if (obj_hud != nullptr)
	{
		...
		{
			if (obj_hud->FindObject("Player") != nullptr)
			{
				...
				GetObjectManager()->FindObject("HP2")->SetTranslation({ -420 / Graphics::checking_zoom, ((float)screen_h / 2 - 30) / Graphics::checking_zoom });			GetObjectManager()->FindObject("HP3")->SetScale({ 50 / Graphics::checking_zoom, 50 / Graphics::checking_zoom });
