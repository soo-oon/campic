...
Object* level1::BuildAndRegisterStaticObject(std::string object_name, vector2 position, vector2 scale)
{
...
Object* level1::BuildAndRegisterDynamicObject(std::string object_name, vector2 position, vector2 scale)
{
	GetObjectManager()->AddObject(object_name);
	GetObjectManager()->FindObject(object_name)->SetScale(scale);
	GetObjectManager()->FindObject(object_name)->SetTranslation(position);
...
void level1::SwordPosition(std::string player_str, std::string sword_str)
{
...
void level1::ForProtoType(Object * object, Object* opponent, float vel_come)
{
...
void level1::snailoption(Object * effect, Object* knife, float angle, float& angle_)
{
	float x_vel, y_vel;
	angle_ += angle;
	//card_velo -= 0.5f;
...
void level1::Enchanted(Object * sword, Object* effect,Object * card1, Object * card2, float dt)
{
	float big =0;
...
void level1::thrust(Object * sword, Object* player, float force)
{
...
void level1::FollowMe(vector2 mouse, Object * flower)
{
...
void level1::Flow_leaf(std::vector<Object*> flower)
{
	for (auto i : flower) {
		float pl = static_cast<float>(rand() % 2);
		if (pl == 1)
			pl = -1;
		else
			pl = 1;
		float p = static_cast<float>(rand() % 2);
		if (p == 1)
			p = -1;
		else
			p = 1;
...
void level1::find(std::string card_)
{
	card_list.push_back(GetObjectManager()->FindObject(card_).get());
}

Object* level1::AwesomeEfftect(std::string stuff, vector2 position)
{
	a_stuff += 1;
...
void level1::ComePlayer(Object * object, Object * opponent, float vel_come)
{
	vector2 come = vector2(object->GetTransform().GetTranslation().x - opponent->GetTransform().GetTranslation().x,
		object->GetTransform().GetTranslation().y - opponent->GetTransform().GetTranslation().y);
	vector2 no_come = normalize(come);
...
void level1::MakeManyEffect(std::string card)
{
...
void level1::Shot(std::string name)
{
