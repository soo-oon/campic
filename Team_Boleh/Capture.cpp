            ...
            if (!obj->IsDifferentZoomSize())
            {
                if (!obj->IsOutSide())
                {
                        ...
                        temp_collision != nullptr)
                    {
                       ...
                       // CollisionChangeZoomInOut_Right(obj.get(), temp_collision, temp_right);
                      //  CollisionChangeZoomInOut_Left(obj.get(), temp_collision, temp_left);
                    }
...
/*
void Capture::CollisionChangeZoomInOut_Right(Object* obj, Collision* collision, Object* temp)
{
    vector2 translation = obj->GetTransform().GetTranslation();
    if (temp != nullptr)
    {
        if ((collision->GetIsRight() || collision->GetIsRightTile()) && collision->GetIsGround())
        {
            float x_pos = temp->GetTransform().GetTranslation().x;
            float x_scale = temp->GetTransform().GetScale().x / 2.0f;
            float obj_scale_x = collision->GetCollisionTransform().GetScale().x / 2.0f;
            obj->SetTranslation({x_pos - x_scale - obj_scale_x - 2.0f, translation.y});
        }
    }
}

void Capture::CollisionChangeZoomInOut_Left(Object* obj, Collision* collision, Object* temp)
{
    vector2 translation = obj->GetTransform().GetTranslation();
    if (temp != nullptr)
    {
        if ((collision->GetIsLeft() || collision->GetIsLeftTile()) && collision->GetIsGround())
        {
            float x_pos = temp->GetTransform().GetTranslation().x;
            float x_scale = temp->GetTransform().GetScale().x / 2.0f;
            float obj_scale_x = collision->GetCollisionTransform().GetScale().x / 2.0f;
